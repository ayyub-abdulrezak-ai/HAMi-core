 ...
 *   /tmp/hami_tbt_<pid>_<dev>.dat   (8 bytes, one double)
 *
...
static void tbt_frac_path(char *buf, size_t len, int dev) {
    snprintf(buf, len, "/tmp/hami_tbt_%d_%d.dat", (int)getpid(), dev);
}

void tbt_set_my_active_frac(int dev, double frac) {
    char path[64];
    tbt_frac_path(path, sizeof(path), dev);
...
double tbt_get_sum_active_fracs(int dev) {
    ...
    char prefix[32];
    snprintf(prefix, sizeof(prefix), "hami_tbt_");
    char suffix[16];
    ...
    LOG_DEBUG("tbt_get_sum_active_fracs dev=%d sum=%.3f count=%d", dev, sum, count);
    return sum > 0.0 ? sum : 1.0;
}

void tbt_cleanup_my_fracs(void) {
    char path[64];
    for (int dev = 0; dev < CUDA_DEVICE_MAX_COUNT; dev++) {
        tbt_frac_path(path, sizeof(path), dev);
        unlink(path);
