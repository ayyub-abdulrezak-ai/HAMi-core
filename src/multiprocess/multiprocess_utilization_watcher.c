...
static int             et_dev_ready[CUDA_DEVICE_MAX_COUNT];
static int             et_stall_count[CUDA_DEVICE_MAX_COUNT];
static volatile int    et_process_count[CUDA_DEVICE_MAX_COUNT];
static pthread_mutex_t et_mutex[CUDA_DEVICE_MAX_COUNT];        // guards sum_active_fracs + stall_count
static double          et_sum_active_fracs[CUDA_DEVICE_MAX_COUNT]; // written by watcher, read by sync hook
static int64_t et_chunk_ns  = 2000000LL; // default 2ms, overridden by ET_CHUNK_MS
static int     et_jitter_pct = 10;         // default ±10%, overridden by ET_JITTER_PCT

static int             et_in_burst[CUDA_DEVICE_MAX_COUNT];
static struct timespec et_burst_start[CUDA_DEVICE_MAX_COUNT];
static unsigned int    et_rand_seed[CUDA_DEVICE_MAX_COUNT];
static int64_t         et_stall_debt_ns[CUDA_DEVICE_MAX_COUNT];  // remaining stall to drip across launches
...
#define ET_STATE_MAX_PROCS 64
#define ET_STATE_PATH_FMT  "/tmp/hami_et_state_%d.dat"
...
typedef struct {
    ...
    et_proc_slot_t    procs[ET_STATE_MAX_PROCS];
} et_shared_state_t;

static et_shared_state_t *et_state[CUDA_DEVICE_MAX_COUNT];
static int                 et_my_slot[CUDA_DEVICE_MAX_COUNT];

static void et_state_init(int dev) {
    char path[64];
    snprintf(path, sizeof(path), ET_STATE_PATH_FMT, dev);
    int fd = open(path, O_RDWR | O_CREAT, 0666);
    if (fd < 0) return;
    if (ftruncate(fd, sizeof(et_shared_state_t)) != 0) { close(fd); return; }
    et_state[dev] = (et_shared_state_t *)mmap(NULL, sizeof(et_shared_state_t),
                         PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    if (et_state[dev] == MAP_FAILED) { et_state[dev] = NULL; return; }

    pid_t my_pid = getpid();
    int n = atomic_load(&et_state[dev]->n_procs);
    for (int i = 0; i < n && i < ET_STATE_MAX_PROCS; i++) {
        if (atomic_load(&et_state[dev]->procs[i].pid) == (int32_t)my_pid) {
            et_my_slot[dev] = i;
            return;
        }
    }
    int slot = atomic_fetch_add(&et_state[dev]->n_procs, 1);
    if (slot >= ET_STATE_MAX_PROCS) { slot = ET_STATE_MAX_PROCS - 1; }
    atomic_store(&et_state[dev]->procs[slot].pid,      (int32_t)my_pid);
    atomic_store(&et_state[dev]->procs[slot].stalling, 0);
    et_my_slot[dev] = slot;
}

static void et_set_stalling(int dev, int val) {
    if (!et_state[dev]) return;
    atomic_store(&et_state[dev]->procs[et_my_slot[dev]].stalling, val);
}

static int et_all_others_stalling(int dev) {
    if (!et_state[dev]) return 0;
    int n = atomic_load(&et_state[dev]->n_procs);
    if (n <= 1) return 0;
    int my_slot = et_my_slot[dev];
    for (int i = 0; i < n && i < ET_STATE_MAX_PROCS; i++) {
        if (i == my_slot) continue;
        if (atomic_load(&et_state[dev]->procs[i].pid) == 0) continue;
        if (!atomic_load(&et_state[dev]->procs[i].stalling)) return 0;
    }
    return 1;
}

void et_shared_state_cleanup(void) {
    pid_t my_pid = getpid();
    for (int dev = 0; dev < CUDA_DEVICE_MAX_COUNT; dev++) {
        if (!et_state[dev]) continue;
        int slot = et_my_slot[dev];
        atomic_store(&et_state[dev]->procs[slot].stalling, 0);
        atomic_store(&et_state[dev]->procs[slot].pid,      0);
        atomic_fetch_sub(&et_state[dev]->n_procs, 1);
        (void)my_pid;
        munmap(et_state[dev], sizeof(et_shared_state_t));
        et_state[dev] = NULL;
    }
}
static int64_t         et_burst_sleep_ns[CUDA_DEVICE_MAX_COUNT]; // sleep already applied during current burst
...
int get_used_gpu_utilization(int *userutil,int *sysprocnum) {
    ...
    for (devi=0;devi<nvmlCounts;devi++){
      ...
      if (res == NVML_SUCCESS) {
        pthread_mutex_lock(&et_mutex[cudadev]);
        et_process_count[cudadev] = (int)infcount;
        pthread_mutex_unlock(&et_mutex[cudadev]);
    ...
    while (1){
        ...
        for (unsigned int dev = 0; dev < device_count && dev < CUDA_DEVICE_MAX_COUNT; dev++) {
            ...
            if (get_experimental_throttler()) {
              // EMA is driven exclusively from sync boundaries (et_sync).
              // The watcher sums active_fracs from shared memory and updates et_sum_active_fracs.
              double sum = et_get_sum_active_fracs(dev);
              pthread_mutex_lock(&et_mutex[dev]);
              et_sum_active_fracs[dev] = sum;
              int stalls = et_stall_count[dev];
              et_stall_count[dev] = 0;
              pthread_mutex_unlock(&et_mutex[dev]);
              LOG_INFO("device %d: [experimental-throttler] limit=%d%% sum_active=%.2f target_active=%.1f%% syncs=%d n_procs=%d\n",
                       dev, cached_sm_limit[dev], sum,
                       cached_sm_limit[dev] / 100.0 * sum * 100.0,
                       stalls, et_process_count[dev]);
            } else {
...
void init_utilization_watcher() {
    ...
    const char *chunk_env = getenv("ET_CHUNK_MS");
    if (chunk_env) et_chunk_ns = (int64_t)(atof(chunk_env) * 1e6);
    const char *jitter_env = getenv("ET_JITTER_PCT");
    if (jitter_env) et_jitter_pct = atoi(jitter_env);

    LOG_MSG("throttle mode: %s", get_experimental_throttler() ? "experimental-throttler" : "NVML-feedback");
    LOG_MSG("tbt chunk=%.0fms jitter=±%d%%", et_chunk_ns / 1e6, et_jitter_pct);
...
static void et_init_device(int device_id) {
    if (et_dev_ready[device_id]) return;
    pthread_mutex_init(&et_mutex[device_id], NULL);
    if (cached_sm_limit[device_id] <= 0 || cached_sm_limit[device_id] >= 100) {
        et_dev_ready[device_id] = 1;
        return;
    }
    // et_sum_active_fracs is intentionally not reset here — the watcher may have
    // already written a valid sum before the first kernel launch triggers init.
    et_stall_count[device_id]    = 0;
    et_process_count[device_id]  = 1;
    et_in_burst[device_id]       = 0;
    et_stall_debt_ns[device_id]  = 0;
    et_burst_sleep_ns[device_id] = 0;
    const char *seed_env = getenv("ET_RAND_SEED");
    unsigned int base_seed = seed_env ? (unsigned int)atoi(seed_env)
                                      : (unsigned int)(getpid() ^ (unsigned int)time(NULL));
    et_rand_seed[device_id] = base_seed ^ (device_id * 2654435761U);
    et_state_init(device_id);
    et_set_my_active_frac(device_id, 1.0);
    et_dev_ready[device_id] = 1;
}

void et_pre_launch(CUstream hStream, int device_id) {
    et_init_device(device_id);
    if (cached_sm_limit[device_id] <= 0 || cached_sm_limit[device_id] >= 100) return;

    if (!et_in_burst[device_id]) {
        clock_gettime(CLOCK_MONOTONIC, &et_burst_start[device_id]);
        et_in_burst[device_id] = 1;
    }
    ...
    if (et_stall_debt_ns[device_id] <= 0) return;

    double limit_frac = cached_sm_limit[device_id] / 100.0;
    int64_t scaled_chunk = (int64_t)(et_chunk_ns * log(1.0 / limit_frac));
    if (scaled_chunk < et_chunk_ns) scaled_chunk = et_chunk_ns;
    int64_t chunk = et_stall_debt_ns[device_id] < scaled_chunk
                  ? et_stall_debt_ns[device_id] : scaled_chunk;
    ...
    if (et_all_others_stalling(device_id)) {
        double r = (double)rand_r(&et_rand_seed[device_id]) / RAND_MAX;
        if (r < limit_frac) {
            // Back-calculation: running instead of stalling counts as spending quota.
            et_stall_debt_ns[device_id] -= chunk;
            if (et_stall_debt_ns[device_id] < 0) et_stall_debt_ns[device_id] = 0;
            return;
        }
    }
    ...
    et_set_stalling(device_id, 1);
    struct timespec ts = {
        .tv_sec  = sleep_ns / 1000000000LL,
        .tv_nsec = sleep_ns % 1000000000LL,
    };
    nanosleep(&ts, NULL);
    et_set_stalling(device_id, 0);
    et_stall_debt_ns[device_id]  -= sleep_ns;
    if (et_stall_debt_ns[device_id] < 0) et_stall_debt_ns[device_id] = 0;
    et_burst_sleep_ns[device_id] += sleep_ns;

    pthread_mutex_lock(&et_mutex[device_id]);
    et_stall_count[device_id]++;
    pthread_mutex_unlock(&et_mutex[device_id]);
}
...
void et_sync(int device_id) {
    if (!et_dev_ready[device_id]) return;
    if (cached_sm_limit[device_id] <= 0 || cached_sm_limit[device_id] >= 100) return;
    if (!et_in_burst[device_id]) return;

    struct timespec now;
    clock_gettime(CLOCK_MONOTONIC, &now);
    int64_t burst_ns = (now.tv_sec  - et_burst_start[device_id].tv_sec)  * 1000000000LL
                     + (now.tv_nsec - et_burst_start[device_id].tv_nsec);

    double limit_frac = cached_sm_limit[device_id] / 100.0;

    pthread_mutex_lock(&et_mutex[device_id]);
    double sum = et_sum_active_fracs[device_id];
    pthread_mutex_unlock(&et_mutex[device_id]);
    if (sum < 1.0) sum = 1.0;

    double target = limit_frac * sum;
    int64_t burst_sleep = et_burst_sleep_ns[device_id];
    if (target < 1.0 && burst_ns > 0) {
        int64_t net_burst_ns = burst_ns - burst_sleep;
        if (net_burst_ns > 0)
            et_stall_debt_ns[device_id] += (int64_t)(net_burst_ns * (1.0 - target) / target);
    }

    double active_frac = target < 1.0 ? target : 1.0;
    et_set_my_active_frac(device_id, active_frac);
    et_in_burst[device_id]       = 0;
    et_burst_sleep_ns[device_id] = 0;

    LOG_INFO("device %d: [et sync] burst=%ldns sleep=%ldns debt=%ldns target=%.1f%% limit=%d%% sum=%.2f\n",
             device_id, burst_ns, burst_sleep,
             et_stall_debt_ns[device_id], target * 100.0, cached_sm_limit[device_id], sum);
}
