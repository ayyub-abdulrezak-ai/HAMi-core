...
The new algorithm (`TIME_BASED_THROTTLE=true`) replaces NVML feedback with direct CUDA event timing. Each process measures its own actual GPU execution time and debits its token budget accordingly. Hardware time-slicing is invisible to this measurement — the throttle enforces limits correctly regardless of how many processes are competing.
