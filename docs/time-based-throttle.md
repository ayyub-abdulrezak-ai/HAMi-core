...
### `[time-based]` — Watcher tick summary

```
device 0: [time-based] limit=30% avg_gpu=33.5% syncs=1 n_procs=3
```
...
### `[tbt sync]` — Sync boundary decision

```
device 0: [tbt sync] burst=490000ns stall=204157ns avg_gpu=70.8% limit=50%
```
...
```
device 0: [tbt ema] burst=490000ns stall=204157ns active_frac=70.6% avg_gpu=70.7% limit=50%
```
